...
export const RouterWrap = ({isAuth, userStore, token, user}:{isAuth: boolean, userStore: IUserData | undefined, token: string | null, user: IUser | null})=>{
	const PrivateRoutes = () => {
...
export function ApplicationRouter({ user, token }: { user: IUser | null; token: string | null }) {
	...
	return load ? (
		<div> Loading ....</div>
	) : <RouterWrap token={token} user={user} isAuth={isAuth} userStore={userStore} />;
}
