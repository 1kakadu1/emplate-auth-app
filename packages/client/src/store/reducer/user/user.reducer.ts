...
export const fetchVerifyUser = createAsyncThunk(
...
const setUser = (
	state: IUserState,
	{ payload }: { payload: { user: IUserData } }
) => {
	state.isAuth = true;
...
export const userSlice = createSlice({
	...
	extraReducers: {
		...
		[fetchLogoutUser.fulfilled.type]: (
			state: IUserState
		) => {
			state.isAuth = false;
		...
		[fetchLogoutUser.rejected.type]: (state: IUserState, { payload }: {payload: string}) => {
			state.error = payload;
		},
