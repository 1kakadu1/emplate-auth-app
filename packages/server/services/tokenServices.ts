...
export class TokenServices{
    generateTokens(payload: UserDto){
        const accessToken = jwt.sign({name: payload.name, email: payload.email, id:  payload.id}, process.env.JWT_ACCESS_SECRET, {expiresIn: "5m"});
        const refreshToken = jwt.sign({name: payload.name, email: payload.email, id:  payload.id}, process.env.JWT_REFRESH_SECRET, {expiresIn: "30d"})
   ...
   async findToken(refreshToken: string | JwtPayload | null){
    console.log("refreshToken", refreshToken);
        if(refreshToken){
            return await DI.tokenRepository.findOne({refreshToken: typeof refreshToken === "string" ? refreshToken : refreshToken.toString()});
        }
        return null;
    }   
