...
export class UserServices {
	async registration(
		email: string,
		password: string,
	): Promise<IRequestData<{ user: UserDto; accessToken: string; refreshToken: string }>> {
	...
	async getUsers({
		...
		limit?: number;
		offset?: number;
	}): Promise<IRequestData<{ users: UserDto[]; count: number }>> {
		const users = await DI.userRepository.findAll({ limit, offset });
		const count = await DI.userRepository.count();

		return {
			data: {
				users: users.map((item) => new UserDto(item)),
				count,
			},
		};
	}
