...
export class UserServices{
    async registration(email: string, password: string): Promise<IRequestData<{user: UserDto, accessToken: string,
        refreshToken: string}>>{
    ...
    async login(email: string, password: string): Promise<IRequestData<{user: UserDto, accessToken: string,
        refreshToken: string}>>{
        const user = await DI.userRepository.findOne({
            email: email
        });
    ...
    async refresh(refreshToken: string | null): Promise<IRequestData<{user: UserDto, accessToken: string,
        refreshToken: string}>>{
        const tokenServices = new TokenServices();
        if(!refreshToken){
        ...
        const user = await DI.userRepository.findOne({id: (userDataToken as UserDto).id});
